...
// Construye la URL final para consultar el tiempo actual con el proveedor
// configurado. Si no hay API key local, devuelve la URL del fallback sin key.
...
async function fetchJson(requestUrl, defaultMessage) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorDetail ? `${defaultMessage} ${errorDetail}` : defaultMessage
    );
  }
...
function mapLocationDetails(locationData) {
  ...
  return {
    city: city?.trim() ?? null,
    locality:
      typeof locationData?.locality === "string" && locationData.locality.trim() !== ""
        ? locationData.locality.trim()
        : null,
    country:
      typeof locationData?.countryName === "string" &&
      locationData.countryName.trim() !== ""
        ? locationData.countryName.trim()
        : null,
  };
}
...
function getGeolocationErrorMessage(error) {
  ...
  if (rawMessage?.toLowerCase().includes("secure")) {
    return "La ubicacion solo funciona en un contexto seguro. Abre la app en http://localhost:5500 o por HTTPS.";
  }
